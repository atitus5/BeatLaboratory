...
song_path = '../data/training' # could make command argument in future
        ...
        if keycode[1] == 'p':
            ...
            if record:
                ...
                if self.writer.active:
                    self.mic_handler.start_training(self.song_data.gems)
                else:
                    self.mic_handler.stop_training()
                    self.training = True
                    print "Training classifier..."
                    start_time = time.clock()
                    self.mic_handler.train_classifier()
                    elapsed_time = time.clock() - start_time
                    print "Trained classifier in %.2f seconds" % elapsed_time
                    self.training = False
...
class SongData(object):
    ...
    def read_data(self, gems_filepath, barlines_filepath):
        ...
        with open(gems_filepath) as f:
            ...
            for g in gems_data:
                self.gems.append((float(g[0]), (int(g[1][-1])) % 3))
            self.gems.sort()
